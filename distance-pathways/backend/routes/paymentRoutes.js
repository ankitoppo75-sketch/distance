...
const { razorpay, isRazorpayConfigured } = require("../utils/razorpay");
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!isRazorpayConfigured) {
      res.status(503);
      throw new Error(
        "Online payments are not set up yet. Please contact the admin, or add Razorpay keys to enable payments."
      );
    }
